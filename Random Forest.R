...
randForestSimpleSplit = function(a.data, a.formula, a.split.ratio) {
...
# returns a vec where first is accuracy and second is MCE
randForestKFoldsSplit = function(a.data, a.formula, a.k) {
...
randForest = function(a.data, a.feature.list, a.num.iter, a.k, a.first.index, a.second.index) {
  # store all results
  ...
  for(i in 1:length(a.feature.list)) {
    ...
    for(j in 1:a.num.iter) {
      ...
      for(z in 1:a.k) {
        res <- randForestSimpleSplit(a.data, f, 0.7)
      ...
      # run k folds
      Kfolds.split <- randForestKFoldsSplit(a.data, f, a.k)
...
MLRandomForest = function(a.datasets.list, a.feature.list, a.num.iter, a.k) {
  ...
  Random.Forest.df <- data.frame(matrix(ncol = 8, nrow = 0))
  colnames(Random.Forest.df) <- c("Algorithm",
                                  "Dataset", 
                                  "Avg.acc.data.split", 
                                  "Avg.acc.kfolds", 
                                  "Avg.mce.data.split", 
  ...
  for(i in 1:length(a.datasets.list)) {
    ...
    for(j in 1:length(datasets)) {
      ...
      # train on dataset
      dataset <- datasets[[j]]
      result.df <- rbind(result.df, randForest(dataset, a.feature.list, a.num.iter, a.k, i, j))
    }
